package data

import (
	"context"
	"database/sql"
	"slices"
	"time"

	"github.com/lib/pq"
)

// permissions in a slice to enable search for later use
type Permissions []string

type PermissionsModel struct {
	DB *sql.DB
}

func (p Permissions) Include(code string) bool {
	return slices.Contains(p, code)
}

func (p *PermissionsModel) GetAllForUser(userID int64) (Permissions, error) {
	query := `
	SELECT permissions.code 
	FROM permissions
	INNER JOIN users_permissions ON
	users_permissions.permission_id = permissions.id
	INNER JOIN users ON users_permissions.user_id = users.id
	WHERE users.id = $1
	`

	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	rows, err := p.DB.QueryContext(ctx, query, userID)
	if err != nil {
		return nil, err
	}

	//release resources after use
	defer rows.Close()

	//store the user permissions in slice
	var persmissions Permissions
	for rows.Next() {
		var permission string
		err := rows.Scan(&permission)
		if err != nil {
			return nil, err
		}
		persmissions = append(persmissions, permission)
	}
	err = rows.Err()
	if err != nil {
		return nil, err
	}
	return persmissions, nil
}

// Add multiple for user. notice: this function accepts multiple permissions (...string)
func (p *PermissionsModel) AddForUser(userID int64, codes ...string) error {
	query := `
	INSERT INTO users_permissions
	SELECT $1, permissions.id
	FROM permissions
	WHERE permissions.code = ANY($2)
	`

	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	//slices need to be converted to arrays to work in psql
	_, err := p.DB.ExecContext(ctx, query, userID, pq.Array(codes))

	return err
}
